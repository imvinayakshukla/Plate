...
const Packets = () => {
    ...
    const handleonSubmit = async (e) => {
        e.preventDefault();
        const response = await fetch(`${window.location.origin}/api/auth/res/packets`, {
            method: 'POST',
            headers: {
    ...
    return (
        ...
        <div className='bg-[#F1F5F9] mt-16 w-[90%] rounded-3xl mx-12 h-[500px] flex justify-around'>
            <form className="mt-24" onSubmit={handleonSubmit}>
                ...
                <button className='bg-green-500 text-white px-4 py-1 rounded-lg mt-10'>Submit</button>
            </form>
